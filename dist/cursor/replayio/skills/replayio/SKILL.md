...
## Cursor Browser Choice

Use Cursor's agent browser only when it is configured to launch Replay Chromium through `AGENT_BROWSER_EXECUTABLE_PATH`. Do not use a preview pane or embedded webview that cannot honor that executable path as the recorded browser run.
...
First inspect the live run with direct agent-browser APIs. Once a recording has uploaded, use the `replay` MCP server tools only when you need deeper Replay-specific debugging beyond direct browser output. Cursor connects to the Replay HTTP MCP server configured in `mcp.json`.
