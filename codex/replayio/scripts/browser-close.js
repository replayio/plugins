...
function runPlaywright(args, sessionName) {
  ...
  if (list.ok) {
    ...
    try {
      ...
      if (upload.ok && /\(failed\)|Upload failed/i.test(upload.stdout || "")) {
        return { command: "upload", result: { ...upload, ok: false, status: 1 } };
      }
      return { command: "upload", result: upload };
    } catch (error) {
...
const sessionName = args.session || state.playwright_session || process.env.REPLAYIO_PLAYWRIGHT_SESSION || "";
const transcodeOptions = {
...
if (output && path.extname(output).toLowerCase() === ".mp4") {
  const stopResult = runPlaywright(["video-stop"], sessionName);
  response.video_stop = stopResult.ok ? "ok" : stopResult.stderr;
}

runPlaywright(["close"], sessionName);
...
response.upload = shouldUpload
  ? uploadRecordings(state.started_at)
