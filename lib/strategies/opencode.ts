...
async function build(context: BuildContext, connector: Connector): Promise<void> {
  ...
  await context.writeText(path.join(out, ".opencode/plugins/replayio.js"), opencodePluginSource);
  await context.copyScript("scripts/playwright_cli.sh", path.join(out, ".opencode/replayio/scripts/playwright_cli.sh"));
  await context.writeText(path.join(out, "AGENTS.md"), context.source.agents);
...
- \`.opencode/plugins/replayio.js\`: OpenCode plugin hooks for Replay recording env vars, \`PWCLI\`, upload-after-close, and idle cleanup.
...
export default {
  name: connectors.opencode.id,
  connector: connectors.opencode,
  build,
};

const opencodePluginSource = `const closePattern = /(playwright|pwcli|PWCLI).*close/;
